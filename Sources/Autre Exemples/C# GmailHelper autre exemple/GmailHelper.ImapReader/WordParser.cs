...
<html xmlns="http://www.w3.org/1999/xhtml" lang="en">
  <head><script type="text/javascript">var NREUMQ=NREUMQ||[];NREUMQ.push(["mark","firstbyte",new Date().getTime()]);</script>
    <title>GmailHelper/GmailHelper.ImapReader/WordParser.cs | Source/SVN | Assembla</title>
...
<input class="main-search" id="q" name="q" onfocus="this.value == 'Search this space' ? this.value = '' : true" size="15" type="text" value="Search this space" />
  ...
  <h1 class="icon-breadcrumb-path"><a href="/code/bniEmC9p0r3P96eJe5aVNr/subversion/nodes?rev=11" class="root">root</a>/<a href="/code/bniEmC9p0r3P96eJe5aVNr/subversion/nodes/GmailHelper?rev=11">GmailHelper</a>/<a href="/code/bniEmC9p0r3P96eJe5aVNr/subversion/nodes/GmailHelper/GmailHelper.ImapReader?rev=11">GmailHelper.ImapReader</a>/<span>WordParser.cs</span><span>    <object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000"
    ...
    <param NAME="FlashVars" value="text=http://subversion.assembla.com/svn/bniEmC9p0r3P96eJe5aVNr/GmailHelper/GmailHelper.ImapReader/WordParser.cs/" />
    ...
    <embed src="/clippy.swf"
           ...
           FlashVars="text=http://subversion.assembla.com/svn/bniEmC9p0r3P96eJe5aVNr/GmailHelper/GmailHelper.ImapReader/WordParser.cs/"
           bgcolor="#FFFFFF"
           wmode="transparent"
    />
    </object>
...
<div class="commit-infobox">
  <div class="commit-options">
    <a href="/code/bniEmC9p0r3P96eJe5aVNr/subversion/node/logs/GmailHelper/GmailHelper.ImapReader/WordParser.cs?rev=11" class="revision-log" rel="nofollow">Revision log</a>
      <div style="margin-right: 10px;" class="small-icon-button">
        <a href="/code/bniEmC9p0r3P96eJe5aVNr/subversion/node/live/11/GmailHelper/GmailHelper.ImapReader/WordParser.cs" class="view-icon" rel="nofollow">View as a web page</a>
      </div>

        <div style="margin-right: 10px;" class="small-icon-button">
    <a class="download-icon" href="#" onclick="nobotGoto('!/code/bniEmC9p0r3P96eJe5aVNr/subversion/nodes/GmailHelper/GmailHelper.ImapReader/WordParser.cs?_format=raw&amp;rev=11'); return false;">Download</a>
  </div>
...
<a href="#ln90" class="block" id="ln90">90</a></pre></th>
...
namespace GmailHelper.ImapReader
{
    ...
    public class WordParser
    {
        ...
        public WordParser(string subject, string body)
        {
            this._subject = subject;
            this._body = body;
        }
        public Dictionary&lt;String, WordFreq&gt; WordStatistics()
        {
            Dictionary&lt;String, WordFreq&gt; result = new Dictionary&lt;string, WordFreq&gt;(StringComparer.InvariantCultureIgnoreCase);
            var subjectFreq = ParseText(this._subject);
            var bodyFreq = ParseText(this._body);
            foreach (var sFreq in subjectFreq)
            {
                result.Add(sFreq.Key, new WordFreq() { SubjectFreq = sFreq.Value });
            }

            foreach (var bFreq in bodyFreq)
            {
                WordFreq wf = new WordFreq() { BodyFreq = 0, SubjectFreq = 0 };
                if (result.TryGetValue(bFreq.Key, out wf))
                {
                    //it exists, so adjust body value,
                    //subject frequency is set in foreach previously
                    wf.BodyFreq = bFreq.Value;
                    //and write it back to result
                    result[bFreq.Key] = wf;
                }
                else
                {
                    //write value for bodyfref, 
                    //subject freq will stay empty
                    wf.BodyFreq = bFreq.Value;
                    result[bFreq.Key] = wf;
                }
            }
        ...
        public Dictionary&lt;string, int&gt; ParseText(string text)
        {
            ...
            foreach (string word in words)
            {
                ...
                if (result.TryGetValue(cleaned, out nrValues))
                {
                    nrValues++;
                    result[cleaned] = nrValues;
                }
                else
                {
