...
<html xmlns="http://www.w3.org/1999/xhtml" lang="en">
  <head><script type="text/javascript">var NREUMQ=NREUMQ||[];NREUMQ.push(["mark","firstbyte",new Date().getTime()]);</script>
    <title>GmailHelper/GmailHelper.Ms.Tests/ImportTests/MailImporterTest.cs | Source/SVN | Assembla</title>
...
<input class="main-search" id="q" name="q" onfocus="this.value == 'Search this space' ? this.value = '' : true" size="15" type="text" value="Search this space" />
  ...
  <h1 class="icon-breadcrumb-path"><a href="/code/bniEmC9p0r3P96eJe5aVNr/subversion/nodes?rev=11" class="root">root</a>/<a href="/code/bniEmC9p0r3P96eJe5aVNr/subversion/nodes/GmailHelper?rev=11">GmailHelper</a>/<a href="/code/bniEmC9p0r3P96eJe5aVNr/subversion/nodes/GmailHelper/GmailHelper.Ms.Tests?rev=11">GmailHelper.Ms.Tests</a>/<a href="/code/bniEmC9p0r3P96eJe5aVNr/subversion/nodes/GmailHelper/GmailHelper.Ms.Tests/ImportTests?rev=11">ImportTests</a>/<span>MailImporterTest.cs</span><span>    <object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000"
    ...
    <param NAME="FlashVars" value="text=http://subversion.assembla.com/svn/bniEmC9p0r3P96eJe5aVNr/GmailHelper/GmailHelper.Ms.Tests/ImportTests/MailImporterTest.cs/" />
    ...
    <embed src="/clippy.swf"
           ...
           FlashVars="text=http://subversion.assembla.com/svn/bniEmC9p0r3P96eJe5aVNr/GmailHelper/GmailHelper.Ms.Tests/ImportTests/MailImporterTest.cs/"
           bgcolor="#FFFFFF"
           wmode="transparent"
    />
    </object>
...
<div class="commit-infobox">
  <div class="commit-options">
    <a href="/code/bniEmC9p0r3P96eJe5aVNr/subversion/node/logs/GmailHelper/GmailHelper.Ms.Tests/ImportTests/MailImporterTest.cs?rev=11" class="revision-log" rel="nofollow">Revision log</a>
      <div style="margin-right: 10px;" class="small-icon-button">
        <a href="/code/bniEmC9p0r3P96eJe5aVNr/subversion/node/live/11/GmailHelper/GmailHelper.Ms.Tests/ImportTests/MailImporterTest.cs" class="view-icon" rel="nofollow">View as a web page</a>
      </div>

        <div style="margin-right: 10px;" class="small-icon-button">
    <a class="download-icon" href="#" onclick="nobotGoto('!/code/bniEmC9p0r3P96eJe5aVNr/subversion/nodes/GmailHelper/GmailHelper.Ms.Tests/ImportTests/MailImporterTest.cs?_format=raw&amp;rev=11'); return false;">Download</a>
  </div>
...
<a href="#ln194" class="block" id="ln194">194</a></pre></th>
            <td><pre class="prettyprint lang-cs">﻿using System.Linq;
using GmailHelper.Data.Linq;
using GmailHelper.ImapReader;
using Microsoft.VisualStudio.TestTools.UnitTesting;
namespace GmailHelper.Ms.Tests
{


    /// &lt;summary&gt;
    ///This is a test class for MailImporterTest and is intended
    ///to contain all MailImporterTest Unit Tests
    ///&lt;/summary&gt;
    [TestClass()]
    public class MailImporterTest
    {
        ...
        {
            emailAdress = &quot;kazazic@gmail.com&quot;; // TODO: Initialize to an appropriate value
            password = &quot;mrd25lll&quot;; // TODO: Initialize to an appropriate value
            ctx = new GmailHelperLinqDataContext();
          
        }

        string emailAdress ; // TODO: Initialize to an appropriate value
        string password ; // TODO: Initialize to an appropriate value
        GmailHelperLinqDataContext ctx ;
        ...
        /// &lt;summary&gt;
        ///A test for ImportMails
        ///&lt;/summary&gt;
        [TestMethod]
        [IgnoreAttribute]
        public void ImportMailsBatchTest()
        {
            var user = ctx.Users.Where(x =&gt; x.Email == emailAdress).First();

            ctx.Mails.DeleteAllOnSubmit(user.Mails.ToList());

            ctx.SubmitChanges();



            MailImporter target = new MailImporter(emailAdress, password); // TODO: Initialize to an appropriate value
            int nrMailsToImport = 5; // TODO: Initialize to an appropriate value
            target.ImportMails(nrMailsToImport);

            //check nr in db
            GmailHelperLinqDataContext ctx2 = new GmailHelperLinqDataContext();
            var user2 = ctx2.Users.Where(x =&gt; x.Email == emailAdress).First();
            Assert.AreEqual(nrMailsToImport, user2.Mails.Count);


        }

        [TestMethod]
        public void can_import_mails973()
        {
            int mailId = 973;
            ...
            //both words comming from 
            Assert.AreEqual(mail.MailWords.Where(x=&gt;x.BodyFrequency&gt;0).Count(), 0);
        ...
        [TestMethod]
        public void can_import_mails9782()
        {
            int mailId = 9782;
        ...
        [TestMethod]
        public void can_import_mails_send_received()
        {
            int mailId = 2;

            //delete mail
            Mail mail = GetMail(mailId);
            Assert.IsNotNull(mail.Sent);
           

        }

        [TestMethod]
        public void can_import_mails_send_even_null()
        {
            int mailId = 1;

            //delete mail
            Mail mail = GetMail(mailId);
            Assert.IsNull(mail.Sent);
        ...
        [TestMethod]
        public void can_import_mails863()
        {
            int mailId = 863;
            ...
            //both words comming from 
            Assert.AreEqual(mail.MailWords.Where(x =&gt; x.BodyFrequency &gt; 0).Count(), 947);


        }


        private Mail GetMail(int mailId)
        {
            var toDel = ctx.Mails.Where(x =&gt; x.ImapId==mailId).FirstOrDefault();

            if(toDel!=null)
            {
                ctx.Mails.DeleteOnSubmit(toDel);
                ctx.SubmitChanges();
            }
            MailImporter target = new MailImporter(emailAdress, password); 
            target.Login();
            target.SelectMailbox();

            var mail =target.ImportSingleMail(mailId);

            target.FinishAndClose();
            return mail;
        }
    }
}</pre></td>
...
NREUMQ.push(["nrfj","beacon-1.newrelic.com","9dfe439095",8763,"Il9dRhNbCVtVQhgXQgBTVkFOWgpTVUMYF1oORw==",0.0,74,new Date().getTime(),"","","","",""])</script></body>
</html>
